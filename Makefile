...
# debug/testing
debug-envoy:
	docker run --net=host --rm -ti --name=envoy-test -v $(shell pwd)/sample/envoy-conf:/config/:ro envoyproxy/envoy:v1.9.0 /usr/local/bin/envoy --v2-config-only -l debug -c /config/envoy-grpc.yaml
